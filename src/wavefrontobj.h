...
struct WavefrontObj {
    ...
    static float rayIntersects(const Ray& ray, const linalg::vec<float,3> v0, const linalg::vec<float,3> v1, const linalg::vec<float,3> v2) {
        const auto epsilon = 1e-07f;

        const auto e1 = v1 - v0;
        const auto e2 = v2 - v0;
        const auto h = linalg::cross(ray.direction, e2);
        const auto a = linalg::dot(e1, h);
        ...
        const auto u = f * linalg::dot(ray.direction, h);

        if (u < 0.0 || u > 1.0) {
            return false;
        }

        const auto q = linalg::cross(s, e1);
        const auto v = f * linalg::dot(ray.direction, q);
    ...
    void forEachTriangle(const std::function<void(const linalg::vec<float,3>& v0, const linalg::vec<float,3>& v1, const linalg::vec<float,3>& v2, const linalg::vec<float,3>& normal)>& lambda) const {
        ...
        for (size_t f = 0; f < shape.mesh.num_face_vertices.size(); f++) {
            ...
            const auto v0 = linalg::vec<float,3>(vx0, vy0, vz0);
            ...
            const auto v1 = linalg::vec<float,3>(vx1, vy1, vz1);
            ...
            const auto v2 = linalg::vec<float,3>(vx2, vy2, vz2);
            ...
            const auto n = linalg::vec<float,3>(nx, ny, nz);
